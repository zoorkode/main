...
function cekNick() {
    ...
    if (storedResult) {
        var response = JSON.parse(storedResult);
        $('#nick').text(response.nickname);
        return; 
    }
    $.ajax({
        method: "GET",
        url: "https://payday.my.id/trueid/game/" + games + "/?id=" + id + "&server=" + server + "&key=KoboStore",
        beforeSend: function () {
...
function showProductDetails(namaProduk, deskripsi, harga, urlProduk) {
  ...
  var serverValueElement = document.getElementById("server_value");
  idValueElement.textContent = document.getElementById("id").value;
  serverValueElement.textContent = document.getElementById("server").value;
  ...
  beliButtonClone.addEventListener("click", function () {
    ...
    if (id.trim() === "") {
        ...
        window.location.href = urlProduk + "?catatan=" + id + server; 
    }
});
